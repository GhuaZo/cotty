...
public class RobotRuntimeListener implements RuntimeListener {
	...
	public void onReceive(CottyManager cottyManager, CottyMessage cottyMessage) {
		if (cottyMessage instanceof FriendMessage) {
			...
			System.out.println("收到好友信息" + cottyMessage.getContent() + "好友UIN"
					+ friendMessage.getUIN());
			/**
			 * 实现一些逻辑
			 */
			...
			System.out.println("收到群信息" + cottyMessage.getContent() + "群UIN"
					+ groupMessage.getGroupUIN() + "作者UIN"
					+ groupMessage.getAuthorUIN());

		} else if (cottyMessage instanceof DiscussMessage) {
			DiscussMessage discussMessage = (DiscussMessage) cottyMessage;
			System.out.println("收到谈论组信息" + cottyMessage.getContent() + "DID"
					+ discussMessage.getDID() + "作者UIN"
					+ discussMessage.getAuthorUIN());
